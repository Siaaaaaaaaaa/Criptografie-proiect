...
- **rnd_numbers.py_v2**: Acest program trimite o cerere către **ANU Quantum Random Number Generator API**, care generează numere aleatoare folosind **fenomene cuantice reale**, nu procese pseudo-aleatoare. API-ul returnează informația în format **JSON**, de aceea se folosește și librăria `json`.
...
- **`requests`** – trimite cereri HTTP și primește răspunsuri de la servere externe, permite verificarea codului de status, preluarea textului sau JSON-ului și tratarea erorilor.
